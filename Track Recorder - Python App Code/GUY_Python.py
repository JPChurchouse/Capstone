...
# Settings
# ---------------------------
DueSerial = serial.Serial('COM42')
DueSerial.baudrate = 115200
DueSerial.bytesize = 8
DueSerial.parity = serial.PARITY_NONE
DueSerial.stopbits = 1
# ---------------------------
NanoSerial = serial.Serial('COM40')
NanoSerial.baudrate = 115200
NanoSerial.bytesize = 8
NanoSerial.parity = serial.PARITY_NONE
NanoSerial.stopbits = 1
# ---------------------------
# INFOS
print("\nChanged Settings")
print('Due Port =', DueSerial.port)
print('Due Baudrate =', DueSerial.baudrate)
print('Due ByteSize =', DueSerial.bytesize)
print('Due Parity = ', DueSerial.parity)
print('Due StopBits =', DueSerial.stopbits)
print('Nano Port =', NanoSerial.port)
print('Nano Baudrate =', NanoSerial.baudrate)
print('Nano ByteSize =', NanoSerial.bytesize)
print('Nano Parity = ', NanoSerial.parity)
print('Nano StopBits =', NanoSerial.stopbits)
...
class LoginScreen(GridLayout):
    def __init__(self, **kwargs):
        ...
        # Title--------------
        self.Title = MDFlatButton(text='      Jakob Jordan Reagan', font_size="25sp")
        ...
        self.X_Control = Slider(min=-148, max=70)
        ...
        # Y----------------
        self.Y_Control = Slider(min=-100, max=70, orientation='vertical')
        ...
        # Solenoids_label--
        self.Solenoids_label = MDRectangleFlatIconButton(text='Gripper -->', icon='swap-horizontal', font_size="20sp",
        ...
        # Solenoids_UP--
        self.Solenoids_Send_Up = MDRectangleFlatIconButton(text="Grip Open", icon='arrow-right', pos_hint={
                                                           "center_x": .5, "center_y": .5}, on_press=self.callback_press_btn_solenoids_up)
        self.add_widget(self.Solenoids_Send_Up)

        # Solenoids_Down--
        self.Solenoids_Send_Down = MDRectangleFlatIconButton(text="Grip Close", icon='arrow-left', pos_hint={
                                                             "center_x": .5, "center_y": .5}, on_press=self.callback_press_btn_solenoids_down)
        self.add_widget(self.Solenoids_Send_Down)

        # Solenoids_Stop--
        self.Solenoids_Send_Down = MDRectangleFlatIconButton(text="Grip STOP", icon='stop', pos_hint={
        ...
        # Vac_Bla_label--
        self.Vac_Bla_label = MDRectangleFlatIconButton(text='Piston -->', icon='swap-vertical', font_size="20sp",
                                                       theme_text_color="Custom", text_color="white", line_color=(0, 0, 0, 0), pos_hint={"center_x": .5, "center_y": .5})
        self.add_widget(self.Vac_Bla_label)

        # Vacuum_On--
        self.Vacuum_On = MDRectangleFlatIconButton(text="Piston Down", icon='arrow-down', pos_hint={
                                                   "center_x": .5, "center_y": .5}, on_press=self.callback_press_btn_vacuum_on)
        self.add_widget(self.Vacuum_On)

        # Blast_On--
        self.Blast_On = MDRectangleFlatIconButton(text="Piston Up", icon='arrow-up', pos_hint={
                                                  "center_x": .5, "center_y": .5}, on_press=self.callback_press_btn_blasr_on)
        self.add_widget(self.Blast_On)

        # Vac_Bla_Off--
        self.Vac_Bla_Off = MDRectangleFlatIconButton(text="Piston STOP", icon='stop', pos_hint={
        ...
        # Vac_Bla_label--
        self.AutoRun_label = MDRectangleFlatIconButton(text='AUTO RUN! -->', icon='play-circle', font_size="20sp",
                                                       theme_text_color="Custom", text_color="cyan",
                                                       line_color=(0, 0, 0, 0),
                                                       pos_hint={"center_x": .5, "center_y": .5})
        self.add_widget(self.AutoRun_label)

        # Vacuum_On--
        self.Run1_button = MDRectangleFlatIconButton(text="Run 1", icon='play-circle', pos_hint={
            "center_x": .5, "center_y": .5}, on_press=self.callback_Run1)
        self.add_widget(self.Run1_button)

        # Blast_On--
        self.FullAuto_button = MDRectangleFlatIconButton(text="Full Auto", icon='play-circle', pos_hint={
            "center_x": .5, "center_y": .5}, on_press=self.callback_FullAuto)
        self.add_widget(self.FullAuto_button)

        # AutoRun_Stop--
        self.AutoRunStop_button = MDRectangleFlatIconButton(text="AutoRun STOP", icon='stop', pos_hint={
            "center_x": .5, "center_y": .5}, on_press=self.callback_AutoRunStop)
        self.add_widget(self.AutoRunStop_button)
    ...
    def callback_press_btn_Home(self, instance):
        print_out('AIR,B')  # Piston Up
        print_out('AIR,U')  # Gripper Open
        time.sleep(0.5)
        print_out('HOME')
    ...
    def callback_Run1(self, instance):
        print_out('HOME')
        time.sleep(5)
        IdentifiedObject = NanoPrint_out('Scale-Tare')
        time.sleep(2)
        NanoPrint_out('Belt-Fwd-2')
        # ---------------------------
        for i in range(5):
            # WEIGH
            IdentifiedObject = NanoPrint_out('Scale-IdentifyObject')
            IdentifiedObject_String = IdentifiedObject.decode("utf-8")
            index = IdentifiedObject_String.index('\r')
            IdentifiedObject_String = IdentifiedObject_String[:index]
            print("Object On Scales = [" + IdentifiedObject_String  + "]")
            # ---------------------------
            # Piston down
            if (IdentifiedObject != b'Err\r\n'):
                if (IdentifiedObject == b'None\r\n'):
                    DropCup_None()
                elif (IdentifiedObject == b'1\r\n'):
                    DropCup_1()
                elif (IdentifiedObject == b'2\r\n'):
                    DropCup_2()
            else:
                DropCup_None()
            #time.sleep(0.5)
...
def print_out(message):
    ...
    DueSerial.write(mdg_str_encode)
    RecivedStr = DueSerial.readline()
    print(RecivedStr)
    return  RecivedStr
...
def print_out_move(value_x, value_y, value_z):
    ...
    # DueSerial.write(b'1,2\r')
    DueSerial.write(move_str_encode)
    RecivedStr = DueSerial.readline()
    print(RecivedStr)


def NanoPrint_out(message):
    msg = message
    mdg_str_encode = str(msg).encode('ascii')
    NanoSerial.write(mdg_str_encode)
    RecivedStr = NanoSerial.readline()
    print(RecivedStr)
    return  RecivedStr
...
class MyApp(MDApp):
    def build(self):
        ...
        self.title = 'Jakob Jordan Reagan - SCARA Arm'
        return LoginScreen()
